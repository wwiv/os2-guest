...
bool set_host_clipboard_internal(char* buf, uint32_t len) {
  int i = 0;
  uint32_t* p = NULL;
  LOG_FUNCTION();
...
bool set_host_clipboard(char* b) {
  ...
  {
    int len = strlen(b);
    set_host_clipboard_internal(b, len);
  }
  return true;
}

static void get_host_clipboard_internal(int32_t len, char* buf) {
...
char* get_host_clipboard() {
  ...
  LOG_FUNCTION();
  len = Backdoor(BACKDOOR_CMD_GET_CLIPBOARD_LEN);
  if (len <= 0) {
    logl(1, "failed to get clipboard len from host\r\n");
    return NULL;
  }
  ...
  get_host_clipboard_internal(len, buf);
  return buf;
}
